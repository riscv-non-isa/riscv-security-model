...
Some RISC-V architectural features require storage for metadata - _architectural metadata storage_. For example,  MTT metadata, metadata for memory tagging, or CHERI metadata.
...
| SR_AMS_005
| Cryptographically protected architectural metadata storage MUST provide privacy protection

| SR_AMS_006
| Cryptographically protected architectural metadata storage MUST provide integrity protection

| SR_AMS_007
| Cryptographically protected architectural metadata storage SHOULD provide replay protection

|===

Architectural metadata needs to be protected against both unauthorized access, and errors (accidental or malicious).

On systems where architectural metadata is stored in external memory, and external memory attacks are in scope (for example, directly accessible or replacable external memory), then cryptographic replay protection is strongly recommended.
