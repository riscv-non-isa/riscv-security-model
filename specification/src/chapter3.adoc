...
===== Supervisor Domain Resource Sharing
...
Supervisor domains allow sharing between domains under the control of M-mode firmware. Trusted Execution environments can require asymmetric sharing models, where one trusted domain has R/W access to other domain's resources
...
designers are free to choose any IOPMP model. Depending on the use case, IOMTT may be an alternative or addition to IOPMP
