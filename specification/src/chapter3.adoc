...
Depending on the use case, MTT may be an acceptable additional or alternative mechanism to the PMP/smepmp
...
| SR_PMP_004
| sPMP SHOULD be used to protect S-mode from lower privilege levels.
| SR_PMP_005
| sPMP MUST be used to protect S-mode from lower privilege levels.
|===

NOTE: Dependent on the use case, it may be recommended or mandatory to protect supervisor mode from lower privilege levels (with or without the H-extension). The MMU may be an additional or alternative mechanism. 
...
| SR_MTT_001
| MTT MUST be used to protect M-mode from lower privilege
levels
...
==== Interrupts, External debug and Performance counters
...
NOTE: Depending on the use case, IOMTT may be an alternative or addition to IOPMP
...
| SR_IOM_002
| IOMTT MUST be used to guarantee that devices assigned to lower privilege levels cannot access resources assigned to M-mode.
...
Use of IOPMP may an addition or alternative adds further protections. For example, a system
