...
* CHERI - a mature technique that uses capability-based memory protection for spatial and temporal memory safety, highly efficient compartmentalisation, and control-flow robustness.  Source code has to be recompiled to capture memory safety properties inherent in the source language.
* Capstone
