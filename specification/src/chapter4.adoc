...
deployment security models. The examples could be extended over time as required.
...
could require that Root devices cannot access memory assigned to Confidential
domain.
...
M-mode can enforce that its own resources are never assigned to another domain.
Use of IOPMP or similar could still add further protections. For example, a system
could require that Root devices cannot be used to access memory assigned to
Confidential domain.
...
a system could require that Root devices cannot be used to access memory assigned
