...
NOTE: The 'Potential RISC-V mitigations' column is informative only and identifies examples of architectural mechanisms that could 
address the corresponding threat. Some entries may correspond to extensions currently under development or under discussion, 
while others are illustrative examples only. Inclusion in this column does not imply an intention or commitment to develop or
standardize any specific extension.  
...
|  Implement power analysis mitigations such as cryptographic masking, constant time code, noise injection, timing decorrelation/randomization, constant power logic designs. +

Implement Data Independent Execution Latency (Zkt, Zvkt) +
Utilize entropy source in randomization (Zkr)
