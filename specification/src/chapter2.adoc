...
This means the system can only access valid derived assets, such as sealing keys, if it is running authorized software. 
...
* Indirect +
The measurement is included in derivation of other assets, for example sealing keys, binding assets to a boot state.
...
Following updates, indirect verification requires a local secure migration process, or a secure provisioning process,  for migrating assets bound to boot state.

A boot process is typically layered allowing software to be measured and verified in stages. Different measurement and verification policies can be employed at different stages. This is discussed further in use case examples later in this specification.
...
| CAT_NNN  
| The source of a local authorization MUST be verified.
|===

For example, a hardware provisioned or securely discovered list of trusted signers.
