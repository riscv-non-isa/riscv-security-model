...
The _trusted compute base (TCB)_ of a component is the minimum amount of
...
The minimum amount of software that always has to be trusted on a system is its _root of trust (RoT)_. A RoT includes fundamental security services, for example:
...
boot flows within a runtime environment, and a SIM can be a root of trust for user identity management. They are usually dedicated trusted subsystems with their own specifications and certification processes. +
 +
...
However, they can also pose a security risk. For example, one workload measuring an operation in a different workload, or an operation by higher privilege software, could be able to reveal assets used in those operations.
...
These types of features rely on measuring resource utilization of workloads, similar to event counters, and optimizing resource allocation policies.
...
| CAT_NNN
| Software in one domains MUST NOT be able to measure software in a different domain, without consent.
...
_Denial of service (DoS)_ is out of scope of this specification. For example, a hosting environment is free to apply their own resource allocation policy to any workloads. Including denying service. This applies in the same way to confidential workloads.
