...
Examples of revealing debug typically includes external debug or boundary scans, dedicated debug builds of software components, or enabling self-hosted debug for a component.

Depending on use case, an attested software component can include debug capabilities managed through an application defined governance process. For example, self-hosted debug enabled following an application specific authorization process. In this case the debug capability, and the associated governance, is part of the trust contract with a reliant party and does not have to be classed as revealing.
...
This state is both attestable and recoverable. For example, revealing debug is enabled
for a domain without compromising another domain or any RoT services.
...
following repair/RMA. This can be viewed as equivalent to starting over from the security
