...
supply chains, each managing _assets_ that need to be protected. The security reference model uses the generic term _domain_ to identify a logically isolated region, with at least some private resources and execution state not accessible to at least some other domain(s). The figure above is only provided for illustration and does not imply or limit the number of domains or the type of use cases a RISC-V system could support. See xref:chapter4.adoc[Use case examples] for more comprehensive and specific examples. 

Logically isolated domains at the software level are typically reflected in logically isolated domains at the system level, where resources including devices can be assigned or restricted to specific domains. For example, device DMA transfers can be restricted to memory assigned to a particular domain.
...
Event counters are commonly used on any system for performance management and resource
allocation.
...
similar to event counters, and optimization of resource allocation policies.
...
At implementation level there can be further distinctions, for example the degree of proximity or vicinity required to execute a remote or a physical attack as defined above. For the purpose of this document we are not making more fine grained distinctions than logical, physical and remote. 
